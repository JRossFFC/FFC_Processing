classdef H9_ge_nav_v1 < PulseSequence
    ...
    methods
        function pulse = H9_ge_nav_v1
            pulse@PulseSequence;
            pulse.pulseSequenceName = 'Inversion Recovery Gradient Echo';
            pulse.pprFile = 'C:\Scanner\H9_scanner\PulseSequences\H9_ge_nav.PPR';
            pulse.dBdt = 7;
            pulse.useMode = 3;
        ...
        function out = AcquisitionFunction(pulse,data)
            ...
            % From here: modifications for image display (JR)
            if (echonumber==1)
            ...
            if (echonumber==1)
                ...
                pulse.userData.dummymat(:,linenumber,current_time,current_field) = nonzeros(double(data.DataBuffer(1,:))) + 1i*nonzeros(double(data.DataBuffer(2,:)));
            ...
            % End of modifications for image display (JR)
            ...
            if pulse.secondEcho % frequency offset correction if the navigator echo is setup
                if (data.Header{8}==0)&&(data.Header{5}==1)  % only correct over the data from receiver 1 and first echo
                    if GetPprParameter(pulse.pprParamList,'offsetcorr',0)  % check  if the temperature correction slider is set
                        ...
                        rawData = double(data.DataBuffer(1,1:data.Header{1},:)) + 1i*double(data.DataBuffer(2,1:data.Header{1},:));
                        ...
                        if successFlag
                            ...
                            if (abs(offsetInHz) > (8.5e6/(2^pulse.scan.DACresolution)))
