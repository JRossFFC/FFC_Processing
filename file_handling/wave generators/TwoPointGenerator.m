classdef TwoPointGenerator < WaveformGenerator
    ...
    methods
        ...
        function wavegen = TwoPointGenerator(varargin)
            wavegen@WaveformGenerator;
            wavegen.name = '2-pts Inversion Recovery';
        end
        ...
        function wavegen = MakeWaveform(wavegen)
            ...
            % add the non-inverted measurements
            if wavegen.Bpol == wavegen.Bread
                wavegen.inversionList = [0;ones(length(Bevo),1)];
                wavegen.Bevo = [wavegen.Bpol;wavegen.Bevo(:)];
            else
                wavegen.inversionList = [0;0;ones(length(Bevo),1)];
