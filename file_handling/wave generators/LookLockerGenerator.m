classdef LookLockerGenerator < WaveformGenerator
    ...
    properties
        NReadout = 1;  %number of times to loop over 
    end
    
    methods
        
        function wavegen = LookLockerGenerator(varargin)
            wavegen@WaveformGenerator;
            wavegen.name = 'NP/PP Look Locker';
...
%             wavegen.Tevo = wavegen.Tevo*ones(1,GetPprParameter(wavegen.waveform.pulse.pprParamList,'no_echoes',2)-1);
        ...
        function wavegen = MakeWaveform(wavegen)
            ...
            if ((wavegen.Tread+wavegen.readDelay)*(wavegen.NReadout)) > 1.5
