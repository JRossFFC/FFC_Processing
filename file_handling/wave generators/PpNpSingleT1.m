classdef PpNpSingleT1 < WaveformGenerator
    ...
    methods
        ...
        function wavegen = PpNpSingleT1(varargin)
        ...
        function wavegen = MakeWaveform(wavegen)
            ...
            % override the T1 estimations
            T1est = 0.2;
            disp('Warning: overriding the T1 estimations to 200 ms.')
            ...
            for indField = 1:length(wavegen.Bevo)
                % generate the list of Tevo depending on Bevo
                if wavegen.Bevo(indField) > 0.13
                    wavegen.Tevo(indField,:) = T1est*logspace(log10(0.01),log10(5),length(wavegen.Tevo(indField,:)));
                else
                    wavegen.Tevo(indField,:) = T1est*logspace(log10(0.01),log10(10),length(wavegen.Tevo(indField,:)));
                end
